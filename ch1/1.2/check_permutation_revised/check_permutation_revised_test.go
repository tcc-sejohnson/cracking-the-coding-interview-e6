package check_permutation_revised_test
...
import (
	...
	cpr "github.com/tcc-sejohnson/cracking-the-coding-interview-e6/ch1/1.2/check_permutation_revised"
)
...
func TestCheckPermutation(t *testing.T) {
	...
	for _, test := range tests {
		got := cpr.IsPermutation(test.FirstInput, test.SecondInput)
		if test.Want != got {
...
func BenchmarkCheckPermutation(b *testing.B) {
	for i := 0; i < b.N; i++ {
		cpr.IsPermutation("the quick brown fox jumped over the lazy dog.", "e epguiy z uotrkjb  chem laervtw  oxfoddqonh.")
	}
}
...
func BenchmarkCheckPermutationOnProgressivelyLongerTrueStrings(b *testing.B) {
	for i := 10; i < 1000000; i = i * 10 {
		...
		b.Run(fmt.Sprintf("MachingStringsOfLength%v", i), func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				cpr.IsPermutation(testString, testString)
			}
		})
	}
}
...
func BenchmarkCheckPermutationOnProgressivelyLongerFalseStrings(b *testing.B) {
	for i := 10; i < 1000000; i = i * 10 {
		...
		b.Run(fmt.Sprintf("MachingStringsOfLength%v", i), func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				cpr.IsPermutation(testString, testNonmatchingString)
			}
		})
	}
}
